...
public class JSONRequest extends StringRequest {
	...
	protected Response<String> parseNetworkResponse(NetworkResponse response) {
		...
		try {
			...
			if (headerMap.containsKey("CACHE")) {
				Log.e("===", "缓存...................");
				return Response.success(responseStr, HttpHeaderParser.parseCacheHeaders(response));
			}
			...
			if (ActivityUtil.getSharedPreferences().getString(Constants.UserName, null) == null) {
				Intent intent = new Intent(GuguApplication.getInstance().getCurrentActivity(), RegisterAndLoginActivity.class);
				GuguApplication.getInstance().getCurrentActivity().startActivityForResult(intent, 0);

			} else {
				Intent intent = new Intent(GuguApplication.getInstance().getCurrentActivity(), LoginActivity.class);
				//intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
				intent.putExtra("FROM", LoginActivity.FROM_TOKEN_EXPIRED);
				GuguApplication.getInstance().getCurrentActivity().startActivityForResult(intent, LoginActivity.FROM_TOKEN_EXPIRED);
			...
			Intent intent = new Intent(GuguApplication.getInstance().getCurrentActivity(), RegisterActivity4.class);
			GuguApplication.getInstance().getCurrentActivity().startActivityForResult(intent, RegisterActivity4.REQUESTCODE_SETINFO);
	...
	private void checkToken(String response) throws TokenExpiredException, NoSetInfoException, Exception {
		...
		} else if (dto.getStatus() == AppResponseStatus.REALNAME) {
